...
login.post('/google', async (req, res) => {
    ...
    Usuario.findOne({email : googleUser.email }, (err, usuarioBD) => {
        ...
        if(usuarioBD){
            ...
            usuario.save((err, usuarioDBNew) =>{
          
                var token = jwt.sign({ usuario: usuarioDBNew }, SEED, { expiresIn: 14400}); // 4 horas
                if( err ) 
                return res.status(400).json({
                    ok: false,
                ...
                return res.status(200).json({
                    ok: true,
                    token,
                    usuario:usuarioDBNew,
                    id: usuarioDBNew._id,
                    menu: obtenerMenu(usuarioDBNew.role)
                });
            });
